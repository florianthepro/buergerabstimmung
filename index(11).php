...
// Baut die Hinweisseite. Vom Ausliefern getrennt, damit der Selbsttest sie pruefen kann.
// Die Seite steht fuer sich allein: kein Stylesheet, kein Skript, kein Symbol von aussen.
function pc_lock_page(): array
{
    ...
    // Gezeigt wird eine Sprache. Der Wunsch aus der Adresse geht vor, dann das
    // Geraet, dann der Browser. Gespeichert wird hier nichts.
    $code = lang_wanted();
    if ($code === '') {
        $code = lang_stored();
    }
    ...
    if (!lang_valid($code)) {
        $code = (string) SW::$cfg['default_lang'];
    }
    ...
    foreach (['de' => 'Deutsch', 'en' => 'English'] as $c => $name) {
        ...
        $switch .= '<a href="' . e(base_path() . '/?lang=' . rawurlencode($c)) . '"'
    ...
    $html = '<!DOCTYPE html><html lang="' . e($code) . '"><head>'
        ...
        . '<title>' . e($t['lock.title']) . ' · ' . e((string) SW::$cfg['app_name']) . '</title>'
        . '<style>' . $css . '</style>'
        . '</head><body>'
