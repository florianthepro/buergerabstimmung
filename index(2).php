...
function sw_regions_merge(array $local, array $wanted): array
{
    ...
    $haveKreis = [];
    $haveLand = [];
    foreach ($local as $land => $list) {
        $haveLand[sw_list_key((string) $land)] = (string) $land;
    ...
    foreach ($wanted as $land => $list) {
        ...
        if (!isset($haveLand[$landKey])) {
            if (count($local) >= SW_MAX_LAENDER) {
                continue;
            }
            $local[$land] = [];
            $haveLand[$landKey] = (string) $land;
            $added[] = ['land' => $land];
        }
        $ziel = $haveLand[$landKey];
        foreach ($list as $kreis) {
...
function cli_selftest(): int
{
    ...
    $check('Gleicher Anzeigename verdraengt keine Kategorie',
        sw_categories_new([['umwelt-klima', 'Umwelt & Klima', 'Environment']],
            [['klima-echt', 'Umwelt und Klima', 'Environment']]) === []);
    $check('Gleicher Kreisname in anderem Land wird nicht doppelt angelegt',
        sw_regions_merge(['Bayern' => ['Landkreis Roth']],
            ['Bayern Sued' => ['Landkreis Roth']])['added'] === [['land' => 'Bayern Sued']]);
    $check('Abweichende Schreibweise eines Landes trifft dasselbe Land',
        sw_regions_merge(['Baden-Württemberg' => ['Ostalbkreis']],
            ['Baden Wuerttemberg' => ['Neuer Kreis']])['list']
