...
const SW_DE = [
    ...
    'flash.vote_withdrawn' => 'Enthaltung gespeichert.',
...
const SW_EN = [
    ...
    'flash.vote_withdrawn' => 'Abstention saved.',
...
function p_vote_values(int $for, int $against): string
{
    $total = $for + $against;
    $pctFor = $total > 0 ? (int) round($for * 100 / $total) : 0;
    $pctAgainst = $total > 0 ? 100 - $pctFor : 0;
...
function v_topic(int $id): void
{
    ...
    if ($archived) {
        ...
        $html .= '<form class="vote-actions" method="post" action="' . e(url('/vote')) . '">' . csrf_field()
            ...
            . '<button type="submit" name="choice" value="none" class="btn vote-btn vote-neutral">' . e(t('vote.neutral')) . '</button>'
            . p_vote_button('against', $myVote)
            . '</form>';
    }
    $html .= p_vote_values($barFor, $barAgainst) . '</section>';
    $head = '';
    if ($user !== null) {
        $head .= fav_menu(
            (int) $user['id'],
