...
.start-legal { display: flex; gap: 0.5rem 1.4rem; flex-wrap: wrap; justify-content: center; margin-top: 3.2rem; font-size: 0.9rem; }
...
function v_start(): void
{
    ...
    echo '</div>'
        ...
        . '<a href="' . e(base_path() . '/about') . '">' . e(t('footer.about')) . '</a>'
        . '<a href="' . e(base_path() . '/imprint') . '">' . e(t('footer.imprint')) . '</a>'
...
// Sprache der eigenstaendigen Seiten. Der Wunsch aus der Adresse geht vor, damit
// der Umschalter auch dann wirkt, wenn auf dem Geraet schon eine Sprache steht.
...
const SW_STATIC_PAGES = [
    '/about'   => ['about.h', ['about.p1', 'about.p2', 'about.p3', 'about.p4', 'about.p5']],
...
function cli_selftest(): int
{
    ...
    $check('Sprachauswahl verlinkt einsprachig, nicht gemischt', (static function (): bool {
        $quelle = (string) file_get_contents(__FILE__);
        if (preg_match('#<nav class="start-legal">(.*?)</nav>#s', $quelle, $m) !== 1) {
            return false;
        }
        // Keine Paarung "deutsch / englisch" mehr, sondern je ein t()-Aufruf.
        return substr_count($m[1], "t('footer.") === 3
            && strpos($m[1], "SW_EN[") === false
            && strpos($m[1], "' / '") === false;
    })());
