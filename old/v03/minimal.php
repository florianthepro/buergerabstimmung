...
{
    $to = query_str('to', 120);
    return preg_match('#^/(topic/\d{1,10}|imprint|privacy)?$#', $to) === 1 && $to !== '' ? $to : '/';
}
...
const SW_DE = [
    ...
    'error.consent' => 'Dafür wird das Sitzungs-Cookie benötigt. Bitte unten zustimmen.',
];
...
const SW_EN = [
    ...
    'error.consent' => 'This needs the session cookie. Please accept below.',
];
...
.consent { position: fixed; left: 0; right: 0; bottom: 0; z-index: 300; background: Canvas; border-top: 1px solid GrayText; padding: .7rem 1rem; display: flex; flex-wrap: wrap; align-items: center; gap: .6rem; font-size: .9rem; }
...
function v_layout(string $title, string $content): string
{
    ...
    $html .= '<main id="main" class="shell site-main">' . $content . '</main>'
        ...
        . '</nav></div></footer>'
        . (consent_given() ? '' : '<div class="consent" role="note"><span>' . e(t('consent.text')) . '</span>'
            . '<span class="consent-actions">'
            . '<a class="btn btn-primary btn-sm" href="' . e(url('/consent?to=' . rawurlencode(SW::$path))) . '">' . e(t('consent.accept')) . '</a>'
            . '<a class="btn btn-outline btn-sm" href="' . e(url('/privacy')) . '">' . e(t('consent.privacy')) . '</a>'
            . '</span></div>')
        . '</body></html>';
    return $html;
}
...
function web_main(): void
{
    ...
    try {
        ...
        $public = $path === '/' || preg_match('#^/topic/\d{1,10}$#', $path) === 1
            || in_array($path, ['/imprint', '/privacy', '/api/topics'], true);
